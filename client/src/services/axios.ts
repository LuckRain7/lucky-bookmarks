...
service.interceptors.request.use(
    config => {
        ...
        // config.headers['token'] = JSON.parse(localStorage.getItem('ZhuChengVue-token'))
        return config
    },
    error => {
...
service.interceptors.response.use(
    response => {
        if (response.data && response.data.code === 401) {
            localStorage.removeItem('ZhuChengVue-token')
            history.push('/login')
        }
